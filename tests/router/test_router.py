...
def test_unknown_endpoint_raises():
    with pytest.raises(CruditConfigError, match="Unknown endpoint"):
        crud_router(model=District, get_db=lambda: None, endpoints=["list", "destroy"])


def test_missing_list_item_schema_raises():
    with pytest.raises(CruditConfigError, match="list_item_schema"):
        crud_router(model=District, get_db=lambda: None, endpoints=["list"])


def test_missing_read_schema_raises():
    with pytest.raises(CruditConfigError, match="read_schema"):
        crud_router(model=District, get_db=lambda: None, endpoints=["read"])
...
def test_options_without_config_raises():
    with pytest.raises(CruditConfigError, match="OptionsConfig"):
        crud_router(
            model=District,
            get_db=lambda: None,
            endpoints=["options"],
...
async def test_list_endpoint_registered(seed, make_client):
    async with make_client(endpoints=["list"], **_FULL) as client:
...
async def test_read_endpoint_registered(seed, make_client):
    async with make_client(endpoints=["read"], **_FULL) as client:
...
async def test_create_endpoint_registered(seed, make_client, cleanup_districts):
    async with make_client(endpoints=["create"], **_FULL) as client:
...
async def test_update_endpoint_registered(seed, make_client):
    async with make_client(endpoints=["update"], **_FULL) as client:
...
async def test_delete_endpoint_registered(router_delete_target, make_client):
    async with make_client(endpoints=["delete"], **_FULL) as client:
...
async def test_reorder_endpoint_registered(seed, make_client):
    async with make_client(endpoints=["reorder"], **_FULL) as client:
